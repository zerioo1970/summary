...
> **【阅读提示】** 本教程前四章重在讲清"是什么、为什么、怎么在本地跑起来、怎么部署到 Windows 服务器上用起来"，从第 5 章开始进入具体功能。关于"前端如何调用这些接口、前后端怎么协作"的内容，集中放在书末**附录 A**，初学者可在读完第 4 章后先翻阅附录 A 建立整体认知，再回到第 5 章深入。标有"（.NET 10 新特性）"的章节是相较旧版本的重要升级点，建议重点掌握。
...
- **内置模型校验**：不用再自己写校验代码，直接用 `[Required]`、`[Range]` 等特性即可，且兼容原生 AoT 编译（第 9 章详讲）。
- **OpenAPI 3.1 支持**：内置生成符合 OpenAPI 3.1 规范的接口文档，能配合 Swagger UI / Scalar 使用（第 11 章）。
- **Server-Sent Events（SSE）**：通过 `TypedResults.ServerSentEvents` 轻松实现服务器实时推送（第 14 章）。
- **原生 AoT 编译增强**：可编译成独立的原生可执行文件，启动更快、体积更小（第 20 章）。
