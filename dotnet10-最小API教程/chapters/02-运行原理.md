...
> **【重点】** 中间件的顺序至关重要。例如 `UseAuthentication` 必须在 `UseAuthorization` 之前，否则"还没搞清你是谁，就先判断你能不能进"，逻辑就错了。第 13 章会深入讲中间件与端点过滤器。
...
- **依赖注入容器（DI Container）**：管理程序里所有"服务"的仓库，需要用哪个服务，它就自动"送货上门"（第 8 章详讲）。
