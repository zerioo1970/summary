...
builder.Services.AddValidation();          // 第 8 章：内置校验
builder.Services.AddProblemDetails();      // 第 12/14 章：标准错误
builder.Services.AddOpenApi();             // 第 10 章：接口文档
...
app.MapHealthChecks("/health");            // 第 15 章：探活

app.Run();

public partial class Program { }           // 第 17 章：便于集成测试
```
...
- `POST` 一个空标题，会看到第 8 章的校验自动返回 400 及错误信息。
