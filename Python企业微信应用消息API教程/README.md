...
> **全书已完成：第 0~18 章 + 附录 A~G**，约 22400 行、187 张图。  
> 新读者请从 [第 0 章](第00章-开始之前.md) 开始（十分钟读完，能省很多回头路）。  
...
> 正文已完成：[`第00章-开始之前.md`](第00章-开始之前.md)

### 0.1 最后交出来的是什么（真实消息 + 真实日志 + 四个命令）
### 0.2 "应用消息"和"群机器人"不是一回事
- 两条路的逐项对比（发给谁 / 鉴权 / 频率 / 能否撤回 / 谁能用）
- 为什么本教程选自建应用；**什么时候该选群机器人**
- 两条不走的路：第三方服务商应用、模拟客户端操作

### 0.3 需要什么基础（以及不需要提前会什么）
### 0.4 这本书怎么读
- 四段式路线图与时间预算；**最短可用路径：第 2 章 → 第 3 章，半小时发出第一条**
- **不能跳的四处**：第 8 章、第 12 章、第 14 章脱敏、第 4.9 节超时
- 每章末尾的"完成标志"；代码为什么写在正文里而不是下载包

### 0.5 这本书的几个写法约定
- 标"实测"的都是真跑出来的（全书 300 多项断言）
- **三类验证来源都标明**：实测 / DB-API 通用 / 依据官方文档
- 我自己写错的地方都留在书里（附错误清单）

### 0.6 全书地图（19 章 + 附录，标注哪些能跳过）
### 0.7 现在开始
...
> 正文已完成：[`附录-速查手册.md`](附录-速查手册.md)（A~G 合成一个文件，速查时 `Ctrl+F` 一次搜完）

## 附录 A　企业微信关键术语速查表
- 七个必须分清的名词（CorpID / Secret / AgentId / UserID / access_token / media_id / msgid）
- 三组最容易混淆：CorpID-AgentId-Secret、UserID-姓名-手机号、**可见范围-可信IP-基础接口许可**

## 附录 B　常用 API 速查表（5 个接口）
- 获取 access_token / 发送应用消息 / **撤回应用消息** / 上传临时素材 / 手机号获取 userid
- 官方频率限制：对同一成员 30 次/分钟、1000 次/小时，**超出部分丢弃不下发**
- **核对官方文档发现的两处代码改动**：漏解析 `unlicenseduser`、补上 `recall()`

## 附录 C　常用消息 JSON 模板
- 6 种模板（①~⑤ 由项目代码真实生成）+ 图片/语音/视频/文件
- 各类型长度上限（注意"字节"与"字符"混用）

## 附录 D　常见返回码与排查顺序
- 七类错误码 → 程序的动作 → 记录状态 → 你要做什么
- 最常遇到的 12 个 errcode + 数据库侧 SQLSTATE 实测表

## 附录 E　SQL Server 脚本
- `01_demo_table.sql` / `02_send_log.sql`（含**唯一索引**）/ `03_sample_data.sql` 全文
- 历史清理（只清 `Sent`、分批删）+ 8 条运维常用查询

## 附录 F　完整配置模板
- `.env.example`（33 项全清单）/ `.gitignore` / `requirements.txt` 全文
- 系统级依赖（unixODBC、msodbcsql18）+ 项目文件清单

## 附录 G　从学习版本到生产版本的功能对照
- 十四个阶段各解决什么问题、不做的后果
- **绝不能省的四样**：token 缓存、发送记录+唯一索引、日志脱敏、请求超时
- 换成自己的业务只需改 3 个文件；两个业务共存怎么拆
