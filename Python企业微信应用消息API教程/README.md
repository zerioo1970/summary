...
> **正文进度：第 1~16 章已完成**，约 19000 行；第 17~19 章与附录待写。  
...
### 17.3 无效 UserID 测试
### 17.4 Token 过期测试
### 17.5 SQL Server 无数据测试
### 17.6 SQL Server 多条数据测试
### 17.7 消息超长测试
### 17.8 定时任务重复触发测试
### 17.9 两个程序同时启动的并发防重测试
### 17.10 网络超时与失败重试测试
### 17.11 程序重启后的防重复测试
### 17.12 敏感信息泄漏检查
### 17.13 最终验收标准
...
### 18.1 获取不到 access_token
### 18.2 HTTP 返回 200，但 `errcode` 不是 0
### 18.3 指定成员收不到消息
### 18.4 一部分成员收到，另一部分没收到
### 18.5 UserID、姓名和手机号混淆
### 18.6 应用可见范围错误
### 18.7 可信 IP 配置问题
### 18.8 Token 频繁获取导致接口受限
### 18.9 中文消息出现乱码
### 18.10 定时任务没有执行
### 18.11 时区导致发送时间不正确
### 18.12 SQL Server 连接失败
### 18.13 ODBC 驱动找不到
### 18.14 同一条消息仍然重复发送
### 18.15 发送成功却没有写入发送日志
