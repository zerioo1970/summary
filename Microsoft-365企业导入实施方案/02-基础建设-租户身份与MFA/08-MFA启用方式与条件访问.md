...
| 方式 | 通俗理解 | 在 E3 下是否可用 | 灵活性 | 本方案定位 |
|---|---|---|---|---|
| **安全默认值** | 一个总开关，打开即获得一套基础保护 | ✅ 可用 | **很低**（只能整体开或关，不能按用户例外） | 仅作切换窗口内的**临时兜底** |
| **按用户 MFA** | 逐个用户设置“启用/强制” | ✅ 可用 | 低（难以管理、易遗漏） | **不使用**（遗留方式，Microsoft 建议迁移） |
| **条件访问** | 按“谁、访问什么、在什么条件下、要求什么”写规则 | ✅ **可用（E3 含 Entra ID P1）** | **高** | **本方案采用** |

> **说明：** 本方案的许可基线是 **Microsoft 365 E3，已包含 Entra ID P1**（第1篇 1.26.2），因此**条件访问从一开始就可用，不存在“要不要采购 P1”的决策**。下面的流程按“直接使用条件访问”编写。

### 2.88.1 实施流程

```mermaid
graph TD
    A[E3 已含 Entra ID P1<br/>直接使用条件访问] --> B[先写策略并设为<br/>仅报告模式]
    B --> C{仅报告结果是否<br/>有意外命中}
    C -->|有| D[修正策略或补例外]
    D --> B
    C -->|无| E[小范围启用<br/>IT 与试点组]
    E --> F[在同一变更窗口内<br/>关闭安全默认值]
    F --> G[分批扩大并监控]
    G --> H[全量启用<br/>紧急访问账号已排除]
```
...
| 没有 Entra ID P1 的租户（**本方案不属于此类**） | 需要按部门分批推行 MFA |
| 希望立刻获得基础保护 | 需要按地点、设备状态控制 |
| 作为条件访问就绪前的**临时兜底** | 需要精细的会话控制 |

> **本方案的用法：** E3 已含 P1，所以安全默认值**不作为最终方案**，只在“条件访问尚未启用”的那段时间里充当兜底，并在切换窗口内关闭。切换要点见 2.88.1。
...
| 控制 | 用途 | E3 是否可用 | 风险 |
|---|---|---|---|
| 按位置（国家/IP） | 阻止业务无关地区的登录 | ✅ 可用 | 出差、VPN、云服务出口 IP 会误伤 |
| 要求合规设备 | 只允许受管设备访问 | ✅ 可用（E3 含 Intune） | 需要 Intune 就绪，否则大面积阻断 |
| **按登录风险 / 用户风险** | 对可疑登录自动加强要求 | ❌ **E3 不含**，需 Entra ID P2 | — |
| 会话控制 | 限制持久登录、限制下载 | ✅ 可用 | 影响用户体验，需沟通 |

> **E3 下的替代做法：** 风险型条件访问需要 Entra ID P2，本方案**不采购**（第1篇 1.26.5）。替代方式是用**固定条件**规则达到近似效果：
>
> | 想防的情况 | E3 下的做法 |
> |---|---|
> | 境外可疑登录 | 用**按位置**策略阻断业务无关国家/地区 |
> | 陌生设备登录 | 用**要求合规设备**或要求已加入设备 |
> | 令牌被盗后长期有效 | 用**会话控制**缩短登录有效期，管理员策略尤其要收紧 |
> | 事后发现可疑登录 | 靠**登录日志巡检**（第6篇 6.45、6.46），而不是自动阻断 |
>
> 差别要向管理层说明清楚：E3 能做的是**按预先写好的条件拦**，做不到**按微软实时算出的风险分自动拦**。可疑登录的发现依赖人工巡检，因此 6.45、6.46 的巡检不能省。
...
- [ ] 已确认采用**条件访问**作为最终方案（E3 含 Entra ID P1，无需额外采购）。
- [ ] 已在管理中心确认本租户**实际具备** Entra ID P1 服务计划，而非仅凭套餐名假设。
- [ ] 已知悉**风险型条件访问在 E3 下不可用**，并已按 2.95 落实固定条件替代规则。
