...
>
> **专题导引：** 本节负责选择设备管理层级、BYOD保护和丢失处置。针对 Microsoft 365 E3 的应用推送、Intune Plan 1/Plan 2、本地组策略、域组策略、三者优缺点与“存量域机过渡、新机纯云”路线，详见[第7篇终端安装与管理专题](../07-终端统一管理-Intune与组策略/README.md)。
...
graph TD
    ...
    C -->|有 层级三 完整设备管理| Z[配置合规策略<br/>并与条件访问联动]
    C -->|没有 先做层级一加二<br/>再逐步演进| Z
    B -->|个人设备为主| G{是否有高敏感数据<br/>或监管要求}
    G -->|是 层级二加对相关人群<br/>叠加层级三| Z
    G -->|否 保持层级二<br/>应用数据保护| Z
```
...
> **必做：** 层级决策要写明理由和适用人群。**许可方面本方案的 E3 已包含 Intune（Plan 1 与 Plan 2）**，层级二和层级三都可实施（第1节 5.13）；实施前请在管理中心确认租户已实际获得 Intune 服务计划。
...
| 无越狱/Root | 移动设备 |
| 补丁在要求范围内 | 降低漏洞风险 |
| 设备未被标记为有风险 | 需与 EDR 风险评分联动，**❌ E3 不含**（需 Defender for Endpoint P2）；本方案不启用该条件 |
...
> **推荐：** "**允许在个人电脑上通过浏览器访问，但禁止下载和同步**"是一个务实的平衡点：员工能应急处理工作，公司文件不落地到不受管的设备上。这依赖条件访问与会话控制，**E3 已包含**（第2篇 2.95），可以直接实施。
...
```mermaid
graph TD
    A[确定层级与人群] --> B[确认租户已获得<br/>Intune 服务计划]
