...
with open("test.jpg", "rb") as f:                 # "rb" 二进制读；with 结束自动关闭文件
    # files 参数会让 requests 使用 multipart/form-data 格式
    files = {"media": ("test.jpg", f, "image/jpeg")}   # 键必须是 media，企业微信规定
    r = requests.post(url, params={"access_token": token, "type": "image"},
                      files=files, timeout=30)    # 上传比发消息慢，超时给足
                                                  # 注意这里是 files= 不是 json=
print(r.json())
```
...
def upload_media_cn(file_path, media_type="file"):
    ...
    with open(file_path, "rb") as f:
        ...
        r = requests.post(
            ...
            headers={"Content-Type": encoder.content_type},  # 必须手工带上，含分隔符
            timeout=60,
        )
...
def file_hash(file_path, chunk_size=1024 * 1024):
    ...
    with open(file_path, "rb") as f:
        while True:
            chunk = f.read(chunk_size)     # 每次只读 1 MB
            if not chunk:                  # 读到空表示文件结束
                break
            h.update(chunk)                # 分块累加，结果与一次性计算相同
    return h.hexdigest()                   # 转成 64 位十六进制字符串
```
...
def upload_media_cn(file_path, media_type="file", max_retry=3):
    ...
    for attempt in range(1, max_retry + 1):
        ...
        try:
            ...
            with open(file_path, "rb") as f:      # 这行必须在循环内，否则重试会读到 0 字节
                encoder = MultipartEncoder(
