...
- **`@Service`**：把这个类注册成 Spring 容器管理的 Bean，这样 Controller 才能注入它。
- **构造器注入 `UserMapper`**：Service **依赖** Mapper 来真正读写数据库。Service 自己不写 SQL，而是"指挥" Mapper 干活。这体现了"Service 调用 Mapper"的层级关系。
