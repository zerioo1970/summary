...
| **第 3 章** | `03-数据库准备与建表.md` | 创建数据库、创建 `t_user` 表、插入测试数据、开启 TCP/IP 端口 | ⏳ 待编写 |
| **第 4 章** | `04-搭建SpringBoot后端项目.md` | 用 Spring Initializr / Gradle 创建后端骨架，运行第一个接口 | ⏳ 待编写 |
| **第 5 章** | `05-集成Mybatis-Flex连接SQLServer.md` | 添加依赖、配置数据源、配置 Mybatis-Flex、连通性测试 | ⏳ 待编写 |
| **第 6 章** | `06-后端查询接口.md` | 编写实体类、Mapper、Service、Controller，实现查询接口 | ⏳ 待编写 |
| **第 7 章** | `07-后端修改接口.md` | 实现按 id 查询 + 更新接口，Postman 测试 | ⏳ 待编写 |
| **第 8 章** | `08-搭建React19前端工程.md` | 用 Vite 创建 React 19 工程，理解目录结构，配置代理 | ⏳ 待编写 |
| **第 9 章** | `09-React19查询程序.md` | 编写「查询程序」：调用后端、渲染表格、加载与错误处理 | ⏳ 待编写 |
| **第 10 章** | `10-React19修改程序.md` | 编写「修改程序」：表单编辑、提交更新、结果反馈 | ⏳ 待编写 |
| **第 11 章** | `11-前后端联调与测试.md` | 跨域（CORS）、联调排错、端到端验证 | ⏳ 待编写 |
| **第 12 章** | `12-常见问题FAQ与部署.md` | 常见报错排查、打包部署建议 | ⏳ 待编写 |

> 📌 本次先交付 **目录 + 第 1 章 + 第 2 章** 的完整详细内容，请你确认是否满足要求，我再继续编写后续章节。
